...
**RESOLVED 2026-06-21 (loop M+11) -- Tibbly confirmed.** Marketing copy
(IA.md, PricingTiers, FreeTierStrip, Hero), ops console branding, plugin
account panel, hub release strategy, licensing/repo split, and all
downstream specs ship under this name. The USPTO TESS check remains
Tom's responsibility before logo/brand spend, but the brand is locked
for documentation and code purposes. Original picked-default and
fallback notes preserved below for context.
...
**RESOLVED 2026-06-21 (loop M+11) -- MIT, per D-10** in DECISION_LOG and
`docs/architecture/LICENSING.md`. The hybrid split locks MIT for the
plugin repo (`RainnWorks/tibbly-plugin`). Original picked-default and
alternatives preserved below for context.
...
**RESOLVED 2026-06-21 (loop M+11) -- option (a) `apps/ops`.** Tom's M+3
review took option (a) of the picked-default ladder; PR #37 merged the
rename and PR #43 shipped the full ops console recast on top. Original
picked-default preserved below for context.
...
**RESOLVED 2026-06-21 (loop M+11) -- merged for compliance.** PR #34
landed for GDPR Art. 15/17 hygiene; PR #41 fixed the PGLite `ANY(array)`
bug it surfaced. Plugin-side panel work continued in parallel (PR #40)
and reused the endpoint shape. Original picked-default preserved below
for context.
...
**RESOLVED 2026-06-21 (loop M+11) -- parallel tracks.** Tom chose a
third option not in the picked-default ladder: both tracks shipped in
parallel during M+5. Plugin account panel landed as PR #40; ops console
recast landed as PR #43. Original picked-default preserved below for
context.
