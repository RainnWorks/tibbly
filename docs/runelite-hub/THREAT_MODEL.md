...
| T5 | RuneLite Hub policy violation — embedded HTTP server exposing player info on localhost | E (Elevation) / I | Was real (PR #11453) | Submission-blocking | The shippable production tree contains zero `ServerSocket(` / `embeddedServer(` references (SECURITY_AUDIT rules 2-3). The old `local/McpServerService` survives in `src/main/kotlin/co/rowm/osrsllm/local/` for in-tree developer use only and is explicitly excluded from the shipped shadowJar by `exclude("co/rowm/osrsllm/local/**")` + `exclude("**/McpServerService*")`, verified every build by `:checkLocalNotInJar` (SECURITY_AUDIT rule 14). The runtime gate (`developerMode=true` AND `localMcpEnabled=true`, both off by default) is now a defense-in-depth secondary check, since the listener class is not even present in the artifact uploaded to the hub. Earlier docs claimed this exclusion was by "package boundary"; that was aspirational and unverified, and was fixed in RAI-40 (see `docs/reviews/plugin-hub-readiness-001.md` blocker 3). | Mitigated |
...
| Build-time `:checkNoSubprocess` | `build.gradle.kts` | `ProcessBuilder` / `Runtime.exec` regression |
| Build-time `:checkLocalNotInJar` | `build.gradle.kts` | `local/` package or `McpServerService*` ever appearing in the shipped jar |
