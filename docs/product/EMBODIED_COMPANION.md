...
- **Q-28**. Visual archetype. ~~Recommendation in this doc: option (e), four starter forms, default to the hooded humanoid in a non-OSRS art style.~~ **RESOLVED 2026-06-21 (RAI-71 pivot + RAI-73 framing fix).** Answer: one visual (the floating Probe) with four voice personalities (dry wiki veteran / soft confused friend / sardonic veteran / earnest helper) and four chassis-tint Probe variants that map to those personalities. The earlier "four starter forms" phrasing implied four different creatures, which the cold-OSRS-player audit (RAI-72) caught as a player-facing lie. The four archetypes live in `apps/backend/src/companion/archetypes.ts`; the four chassis tints live in `CompanionSpriteAtlas.Variant` in the plugin. See [COMPANION_VISUAL_BIBLE.md §2](./COMPANION_VISUAL_BIBLE.md#2-one-visual-four-personalities-probe--voices) for the source of truth.
